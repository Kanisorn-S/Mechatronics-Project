...
if writeValues==True: np.save(os.path.join(savedir, 'inverse_newcam_mtx.npy'), inverse_newcam_mtx)
...
print(rvec1)
if writeValues==True: np.save(os.path.join(savedir, 'rvec1.npy'), rvec1)

print("pnp tvec1 - Translation")
print(tvec1)
if writeValues==True: np.save(os.path.join(savedir, 'tvec1.npy'), tvec1)

print("R - rodrigues vecs")
R_mtx, jac=cv2.Rodrigues(rvec1)
print(R_mtx)
if writeValues==True: np.save(os.path.join(savedir, 'R_mtx.npy'), R_mtx)

print("R|t - Extrinsic Matrix")
Rt=np.column_stack((R_mtx,tvec1))
print(Rt)
if writeValues==True: np.save(os.path.join(savedir, 'Rt.npy'), Rt)

print("newCamMtx*R|t - Projection Matrix")
P_mtx=newcam_mtx.dot(Rt)
print(P_mtx)
if writeValues==True: np.save(os.path.join(savedir, 'P_mtx.npy'), P_mtx)
...
for i in range(0,total_points_used):
    ...
    s_describe[i]=s
    if writeValues==True: np.save(os.path.join(savedir, 's_arr.npy'), s_arr)
